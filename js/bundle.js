...
//# sourceMappingURL=lenis.min.js.map﻿/* Film-grain texture generator → applied to #hero-grain */
(function () {
  function makeGrain() {
...
if (window.__IH_FORCE_STATIC || !window.gsap || !window.ScrollTrigger) {
  document.addEventListener('DOMContentLoaded', function() {
    try { if (typeof renderAll === 'function') renderAll(); } catch(e) {}
    document.body.classList.add('zoom-hover', 'motion-fallback');
    document.querySelectorAll('#loader, .ld-shutter').forEach(function(el) {
    ...
    var close = document.getElementById('navoClose');
    function setMenu(open) {
      ...
      if (open) {
        var first = overlay.querySelector('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])');
    ...
    document.querySelectorAll('.navo-link').forEach(function(link) {
      link.addEventListener('click', function() { setMenu(false); });
    });

    var fallbackGlyphs = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789·—×ø№';
    function fallbackScramble(el, duration) {
      var original = el.textContent;
      if (!original.trim()) return;
      var total = Math.max(12, Math.round((duration || 0.45) * 60));
      var frame = 0;
      function run() {
        var revealed = Math.floor((frame / total) * original.length);
        el.textContent = original.split('').map(function(ch, i) {
          if (' ®—·()№'.indexOf(ch) >= 0) return ch;
          return i < revealed ? ch : fallbackGlyphs[Math.floor(Math.random() * fallbackGlyphs.length)];
        }).join('');
    ...
    document.querySelectorAll('.js-partner').forEach(function(row) {
      var name = row.querySelector('.pcr-name, .partner-name');
      if (!name) return;
      var running = false;
      row.addEventListener('mouseenter', function() {
        if (running) return;
        running = true;
        fallbackScramble(name, 0.45);
        setTimeout(function() { running = false; }, 500);
      });
    });
    document.querySelectorAll('.navo-link').forEach(function(link) {
      var label = typeof getNavScrambleTarget === 'function'
        ? getNavScrambleTarget(link)
        : link.querySelector('.li');
      if (!label) return;
      var running = false;
      link.addEventListener('mouseenter', function() {
        if (running) return;
        running = true;
        fallbackScramble(label, 0.35);
  ...
  console.warn('[IH] GSAP/ScrollTrigger unavailable: static fallback enabled.');
} else {
...
document.addEventListener('DOMContentLoaded', () => {
  if (window.__IH_FORCE_STATIC || !window.gsap || !window.ScrollTrigger) return;
  renderAll();
  applyTweaks();
  buildTweaksPanel();
...
function initLoader() {
  ...
  gsap.timeline({ onComplete: boot })
    .to(obj, { v: 100, duration: TOUCH ? 1.15 : 1.55, ease:'power2.inOut',
        onUpdate() { numEl.textContent = String(Math.round(obj.v)).padStart(2,'0'); } })
    .to('.ld-brand', { opacity: 0, duration: TOUCH ? .2 : .35, ease:'power2.in' }, TOUCH ? '-=0.28' : '-=0.5')
    .to('.ld-num',   { y:'-115%', opacity: 0, duration: TOUCH ? .42 : .65, ease:'power3.inOut' }, TOUCH ? '-=0.18' : '-=0.2')
    .to('.ld-shutter', { scaleY: 1, duration: TOUCH ? .42 : .6, ease:'power4.inOut', transformOrigin:'bottom' }, TOUCH ? '-=0.32' : '-=0.45')
    .set('#loader', { display:'none' })
    .set('.ld-shutter', { transformOrigin:'top' })
    .to('.ld-shutter', { scaleY: 0, duration: TOUCH ? .48 : .65, ease:'power4.inOut' });
}
...
function initAmbient() {
  ['.mani-photo', '.foot-vis img', '.split-photo img',
   '.narr-photo img', '.work-card-img', '.wed-img', '.foot-bg img'].forEach(sel =>
...
function initScroll() {
  ...
  ScrollTrigger.create({ trigger:'#stats', start:'top 72%', once: true, onEnter: () => {
    STATS.forEach(s => {
      ...
      gsap.to(o, { v: s.t, duration: 2, ease:'power2.out',
        onUpdate() { el.innerHTML = o.v.toFixed(s.dec).replace('.', ',') + s.suf; } });
    });
  }});
  });
  ...
  if (ctaBg) {
    gsap.to(ctaBg, { yPercent: -14, ease:'none',
  ...
  seg('social', () => {
  const follow = document.querySelector('.social-follow');
  if (follow) {
    gsap.from(follow, { opacity: 0, y: 12, duration: .5, ease:'power2.out',
      scrollTrigger: { trigger: follow, start:'top 92%' } });
  }
  });
...
function getNavScrambleTarget(link) {
  const span = link.querySelector('.li');
  if (!span) return null;
  let target = span.querySelector('.navo-scramble-text');
  if (target) return target;

  const idx = span.querySelector('.idx');
  let label = '';
  Array.from(span.childNodes).forEach(node => {
    if (node.nodeType === 3) label += node.nodeValue;
  });
  label = label.trim();
  if (!label && idx) label = span.textContent.replace(idx.textContent, '').trim();
  ...
  target.className = 'navo-scramble-text';
...
function initScramble() {
  ...
  document.querySelectorAll('.js-partner').forEach(row => {
    const nm = row.querySelector('.pcr-name, .partner-name');
    if (!nm) return;
    let running = false;
  ...
  document.querySelectorAll('.navo-link').forEach(l => {
    ...
    l.addEventListener('mouseenter', () => {
      if (running) return;
      running = true;
      const target = getNavScrambleTarget(l);
      if (target) scrambleEl(target, { duration: 0.35 });
...
function buildTweaksPanel() {
  ...
  p.innerHTML = `
    <div class="tw-head">
      ...
      <button class="tw-close" id="twClose" aria-label="Tweaks bezárása">✕</button>
    </div>
    ...
    <div class="tw-swatches" data-key="accent">
      ${accents.map(c => `<button class="tw-sw" data-v="${c}" style="background:${c}" aria-label="Akcent szín ${c}"></button>`).join('')}
    </div>
    <div class="tw-sec">WebGL háttér</div>
    <div class="tw-toggle">
      <span>Hero noise-field</span>
      <button class="tw-switch" data-key="webgl" aria-label="WebGL háttér kapcsolása"></button>
    </div>
    <div class="tw-sec">Filmszemcse</div>
    <div class="tw-toggle">
      <span>Grain overlay</span>
      <button class="tw-switch" data-key="grain" aria-label="Filmszemcse kapcsolása"></button>
    </div>`;
...
/* ─── LOADER TYPEWRITER ─────────────────────────────────────────
   ...
   ─────────────────────────────────────────────────────────────── */
if (window.__IH_FORCE_STATIC || !window.gsap || !window.ScrollTrigger || typeof initLoader !== 'function') {
  console.warn('[IH v7] Motion patch skipped: GSAP fallback is active.');
} else {
...
initScramble = function() {
  ...
  document.querySelectorAll('.navo-link').forEach(function(l) {
    ...
    l.addEventListener('mouseenter', function() {
      if (running) return;
      running = true;
      var target = typeof getNavScrambleTarget === 'function'
        ? getNavScrambleTarget(l)
        : l.querySelector('.li');
      if (target) scrambleEl(target, { duration: 0.35 });
      setTimeout(function() { running = false; }, 400);
    });
...
/* ─── PARTNER SOR: PROJEKT-FOTÓ HOVER REVEAL ────────────────────
   ...
   Asset megjegyzés: lok?lis IH k?pek —
...
function initPartnerPreview() {
  ...
  /* Képmapping — a Kollaborációk HTML-ben lévő aktuális neveihez kötve.
     Fontos: a hover csak akkor jelenik meg, ha a .pcr-name szövege pontosan
     szerepel ebben a mapben; ezért az új partnernevekhez is van kép. */
  /* Egységes nyelv: minden partnerhez 35mm-es projektfotó (logó a marquee-ban van) */
  var PARTNER_IMGS = {
    'HBO':            'assets/ih-all/04_projects/pogany-indulo/timeline-1_01_00_58_16.webp',
    'Beton.Hofi':     'assets/ih-all/04_projects/hofi-tarr-bela/357_0006.webp',
    'Cenzúra':        'assets/ih-all/04_projects/cenzura-xxl/still-2025-10-31-104632_1.25.1.webp',
    'Sneakerness':    'assets/ih-all/04_projects/kepes-interjuk/14.webp',
    'Budapest Film':  'assets/ih-all/04_projects/insta-karuzel/biff/1.webp',
    'Budapest Park':  'assets/ih-all/04_projects/rendszerbonto/dji_20260410195726_0870_d.webp',
    'Dugattyús':      'assets/ih-all/04_projects/offline/dsc08223.webp',
    'Bródy House':    'assets/ih-all/02_about/about_studio_chair_lamp.webp',
    'Friss Hús':      'assets/ih-all/04_projects/kepes-interjuk/17.webp',
  };
  ...
  document.querySelectorAll('.js-partner').forEach(function(row) {
    ...
    var src  = PARTNER_IMGS[name] ? W(PARTNER_IMGS[name], 800) : undefined;
...
/* ─── v7.5 — LOADER SESSION-SKIP ─────────────────────────────────
   Első látogatáskor a teljes loader fut (typewriter + 00→100 + redőny).
   Ugyanazon munkameneten belüli visszatéréskor 0,25s fade — a
   kinematikus belépő megmarad, az ismételt várakozás nem. */
(function() {
  if (typeof initLoader !== 'function' || typeof boot !== 'function' || !window.gsap) return;
  var _fullLoader = initLoader;
  initLoader = function() {
    var seen = false;
    try {
      seen = sessionStorage.getItem('ih_v7_seen') === '1';
      sessionStorage.setItem('ih_v7_seen', '1');
    } catch (e) {}
    if (!seen) { _fullLoader(); return; }
    var num = document.querySelector('.ld-num .n');
    if (num) num.textContent = '100';
    gsap.timeline({ onComplete: boot })
      .to('#loader', { opacity: 0, duration: 0.25, ease: 'power2.out' })
      .set('#loader', { display: 'none' });
  };
})();
