# Isten Hozott® — Weboldal · Projekt-kézikönyv (handoff Claude Code-hoz)

> Ez a fájl a projekt belépőpontja. Ha Claude Code-ban folytatod a munkát, ezt
...
> `Homepage v7.html`** (a v4/v5/v6 régebbi iterációk, lásd lent).
...
landing (`Homepage v7.html`) + egy munkák-aloldal (`work/index.html`) + tíz
projekt-részletoldal (`work/*.html`).
...
```
/
├── CLAUDE.md                 ← EZ A FÁJL (projektdoku)
├── MOTION-SYSTEM.md          ← a motion-rendszer mélységi leírása (érdemes elolvasni)
│
├── Homepage v7.html          ← ✅ KANONIKUS főoldal (ezt fejleszd)
├── Homepage v6.html          ← régi iteráció (megőrzött)
...
│   ├── beton-hofi-tarr-bela.html
│   ├── cnzura-xxl.html
│   ├── fekete-giorgio-5-kotelezo.html
│   ├── kepes-interjuk.html
│   ├── offline-dugattyus.html  ← Offline case study (a főoldal kártyája ide linkel)
│   ├── pogany-indulo-hbo.html
│   ├── rendszerbonto.html
│   ├── spotlight.html
│   └── vizualis-esszek.html
│
├── styles/
│   ├── homepage-bundle.css   ← ✅ ÉLŐ artefakt: v6+v7 css konkatenálva — az index.html EZT tölti;
│   │                            módosításkor a forrás css-be IS írd be ugyanazt (v8 EXTRAS blokk a végén)
│   ├── homepage-v6.css       ← ✅ a v7 alap stíluslapja (tokenek itt: :root)
│   ├── homepage-v7.css       ← ✅ a v7 felülírásai / új szekciók stílusa
...
│   ├── v7-app.js             ← ⚠ v7 patch FORRÁS — a bundle.js-beli másolata azóta driftelt!
│   ├── v7-extras.js          ← ✅ v8 extras (önálló réteg, a bundle UTÁN töltődik): oldalátmenet
│   │                            (shutter-wipe), esszé drag-szalag + IG CTA, fejezet-sín,
│   │                            napszakos loader-üdvözlés, title-csere, console egg
│   │                            (a WebGL displacement-hover KIKERÜLT — user kérésre a sima
│   │                            grayscale→szín hover maradt a works-kártyákon)
│   ├── bundle.js             ← ✅ ÉLŐ artefakt: GSAP+ScrollTrigger+Lenis+grain+v6-data+v6-fx+v6-app
│   │                            +v7-app konkatenálva — az index.html EZT tölti, nem a forrásfájlokat
│   ├── v4-grain.js           ← filmszemcse-overlay generátor (v7 is használja)
...
> **Megőrzött régi verziók:** a `v4`/`v5`/`v6`/sima nevű fájlok korábbi iterációk,
> szándékosan megtartva referenciának. Új munkát a **v7** rétegen végezz. Ha
> egy régi verzió már nem kell, törölhető — de a v7 nem hivatkozik rájuk.
...
A fontok mind lokálisak: a Big Shoulders WOFF2+TTF a `fonts/`-ban, az
**Andada Pro + Space Mono** szintén lokálisan hostolt (`styles/fonts-local.css`
+ `fonts/gf/*.woff2`). A fotók lokális webp-k (`assets/ih-all/`) — lásd §8.
...
| `IMG{}` + `W(url,width)` | lokális webp-k + variáns-helper: a `W` a generált `-800.webp`/`-1600.webp` fájlra mutat (generálás: `_optimize_images.py`) | — |
...
- **A fotók lokálisak** (`assets/ih-all/`), és minden hivatkozott webp-hez van
  generált `-800.webp` / `-1600.webp` átméretezett változat (`_optimize_images.py`
  állítja elő). Új kép hozzáadásakor futtasd újra a scriptet, és `srcset`-tel
  hivatkozz. A design system szerint a fotó
  **mindig meleg, szemcsés, deszaturált** (35mm), és **mindig scrim alatt** van —
  ne lebegjen szöveg nyers fotón.
- A `signature.webm` **be van kötve** a v7 footerébe (`.foot-sign`): screen blend
  + brightness filter, IntersectionObserverre játszik le egyszer. A
  `handwriting-istenhozott.mp4` továbbra is tartalék.
- A logó-marquee-k (footer + Kollaborációk) **mind SVG-t** futtatnak
  (`assets/ih-all/03_partner_logos/svg/*.svg`): HBO, Budapest Park, Friss Hús
  hivatalos vektor; Sneakerness hivatalos „S" mark megvan (`sneakerness-mark.svg`),
  de a sávban a generált wordmark fut. Beton.Hofi, Cenzúra, Dugattyús, Bródy House,
  Budapest Film, Sneakerness wordmarkjait a `_make_wordmark_svgs.py` generálja
  (Big Shoulders Bold → path). Színezés CSS-ből: `.logo-mq-svg{filter:brightness(0) invert(.92)}`.
  A régi `*-trim.webp` fájlok megmaradtak tartaléknak.
...
- **Stílus / tokenek** → `styles/homepage-v6.css` (`:root`) + `styles/homepage-v7.css` (v7 felülírások)
- **Motion / interakció** → `js/v6-app.js` + `js/v7-app.js` (+ `MOTION-SYSTEM.md` olvasd előbb)
- **WebGL / canvas / scroll-színek** → `js/v6-fx.js`
- **Work aloldalak** → `work/index.html`, `work/*.html`, `styles/work.css`, `js/page-core.js`
